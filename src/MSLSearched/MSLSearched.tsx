...
import RefeshIcon from "../icons/RefeshIcon";
...
export default function MSLSearched({ onClose, setIsSearched }: Props) {
  ...
  return (
    <PopupWrapper>
      ...
      <MSLProfile />
      <div className="flex justify-center gap-2 items-center pb-7">
        <p
          onClick={() => sendEmail(data?.email)}
          className={`cursor-pointer text-base flex justify-center items-center tracking-[0.5px] text-white border-2 border-[#00000000] bg-[#00000040] w-[40%] py-1 rounded-[39px]  text-[0.75rem] font-[600]`}
          aria-label="Open mail to window to email MSL"
        >
          Contact MSL
        </p>
        <p
          className={`cursor-pointer text-base flex justify-center items-center tracking-[0.5px] text-white border-2 border-[#ffffff] w-[40%] py-1 rounded-[39px]  text-[0.75rem] font-[600]`}
          onClick={() => setIsSearched(false)}
        >
          <RefeshIcon /> New Search
        </p>
      </div>
      <div className="font-light text-base flex justify-center items-center tracking-[0.5px] text-white bg-[#00000040] w-full py-4   gap-1 cursor-pointer">
        <a
          className="underline text-[1rem] font-[500] cursor-pointer leading-relaxed tracking-wide"
          onClick={handleRegister}
        >
          Register
        </a>
        <p className="te text-[1rem]">or</p>
        <a
          className="underline text-[1rem] font-[500] cursor-pointer leading-relaxed tracking-wide"
